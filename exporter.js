...
function importChatficJson(){
    const importCodeModalTextarea = document.getElementById('importCodeModalTextarea');
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    importCodeModal = new bootstrap.Modal(document.getElementById("importCodeModal"));
});
...
`;
    for (let key in fic.characters) {
        ...
        if(character.hasOwnProperty('color') && character.color && character.color.length>1){
            chatficbasic+="> characters/"+key+"/color : "+character.color+"\n";
        }
        if(character.hasOwnProperty('avatar') && character.avatar && character.avatar.length>1){
            chatficbasic+="> characters/"+key+"/avatar : "+character.avatar+"\n";
        }
    }
    ...
    fic.pages.forEach(page => {
        ...
        page.messages.forEach(message => {
            const povText = (message.from!="player" && message.side==2) ? "(pov)" : "";
            const chatroomText = message.chatroom != latestChatroom && message.chatroom != fic.characters[message.from].name ? "("+message.chatroom+")" : "";
            const imageText = message.multimedia && (!message.multimedia.toLowerCase().endsWith(".mp4") && !message.multimedia.toLowerCase().endsWith(".webm")) ? " ![IMAGE]("+message.multimedia+")" : "";
            const videoText = message.multimedia && (message.multimedia.toLowerCase().endsWith(".mp4") || message.multimedia.toLowerCase().endsWith(".webm")) ? " ![VIDEO]("+message.multimedia+")" : "";
            chatficbasic+=`${message.from}${chatroomText}${povText}: ${imageText}${videoText}${message.message ? " "+message.message.replace("\n"," "):""}\n`;
